...
function getCacheKey(ingredients: Ingredient[]): string {
  return ingredients
    .map((i) =>
...
- packagePrice: what a typical US shopper pays for ONE normal grocery package of that item (e.g., $3.49 for 1 bunch cilantro, $4.99 for 16oz pasta)
- adjustedPrice: the portion cost for only the recipe's quantity (e.g., if package is $3.49 for 100g and recipe needs 20g, adjustedPrice = 0.70)
- packageSizeText: short description of the typical package you priced (e.g., "1 bunch", "16 oz box", "32 fl oz bottle", "1 lb bag")
- matchTitle: the generic grocery-store product name (lowercase, no brand)
- confidence: 0.0 to 1.0 — how confident you are in this estimate
- explanation: one short sentence about the price source/logic

Rules for adjustedPrice:
- A pinch of salt, 1 bay leaf, or 1/4 tsp of a common spice is pennies — use 0.02-0.15
- Small amounts of pantry staples (flour, sugar, oil) used in a recipe: proportion of package price
- Fresh produce priced by count or bunch: scale appropriately (e.g., 2 scallions out of a bunch of 8 is ~$0.50 from a $1.99 bunch)
- Meat/seafood: price for the exact weight needed
- Always return a realistic USD number, not null, unless the quantity is genuinely unspecifiable
- Round to 2 decimals

Rules for packagePrice:
- Use the MOST COMMON package size the ingredient sells in
- Base prices on 2024-2025 national averages; no coupons or sales

When to return null for adjustedPrice:
- Only if quantity is truly unknowable (e.g., "to taste" with no practical equivalent)
- Otherwise always give a number — err on the side of a small reasonable estimate
...
Output ONLY valid JSON, no commentary, no markdown fences. Format:
{
  "estimates": [
    {
      ...
      "packageSizeText": "16 oz bag",
      "packagePrice": 4.99,
      "adjustedPrice": 0.62,
      "confidence": 0.85,
      "explanation": "based on 2 oz of a 16 oz pasta bag at typical $4.99",
  ...
  const message = await anthropic.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `Price every ingredient below. Return ONE estimate object per ingredient, matching ingredientId exactly. Do not skip any ingredient.\n\nIngredients:\n${JSON.stringify(userPayload, null, 2)}`,
      },
    ],
  });
...
export async function estimateIngredientsWithClaude(
  ...
  try {
    const cacheKey = getCacheKey(pricable);
    const priceMap = await getCached(claudePricingCache, cacheKey, () =>
      callClaudeForPricing(pricable)
    );
    ...
    for (const ingredient of pricable) {
      ...
      results.set(ingredient.id, buildEstimateFromClaude(ingredient, entry));
    }
  } catch (error) {
