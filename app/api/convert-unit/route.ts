...
type ExactUnit = keyof typeof EXACT_UNIT_FACTORS;
...
function normalizeUnit(value: string): ExactUnit | null {
...
function parseQuantityExpression(
  value: string
): { values: number[]; isRange: boolean } | null {
  const normalized = normalizeQuantityText(value).replace(/,/g, "");
  if (!normalized) return null;
...
function formatConvertedExpression(values: number[], isRange: boolean): string {
...
function resolvePortionVolumeUnit(
  ...
  for (const text of texts) {
    const directUnit = normalizeUnit(text);
...
async function fetchUsdaFoodDetails(
  fdcId: number,
  apiKey: string
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (parsedQuantity === null) {
      return NextResponse.json(
        { error: "Could not parse quantity for conversion" },
        { status: 422 }
      );
    }
    ...
    if (!normalizedFromUnit || !normalizedToUnit) {
      return NextResponse.json(
        { error: "Unsupported unit for conversion" },
        { status: 422 }
      );
    }
    ...
    if (fromMetadata.category === toMetadata.category) {
      ...
      return NextResponse.json({
        convertedQuantity: formatConvertedExpression(
          convertedValues,
          parsedQuantity.isRange
        ),
        fromUnit,
        toUnit,
      });
    ...
    const densityMatch = await findUsdaDensityMatch(
      ingredientName,
      preferredVolumeUnit,
      apiKey
    );
    ...
    const convertedValues = parsedQuantity.values.map((value) => {
      if (fromMetadata.category === "volume") {
        ...
        const sourceGrams =
          sourceVolumeMilliliters * densityMatch.gramsPerMilliliter;
        return sourceGrams / toMetadata.factor;
      }

      const sourceGrams = value * fromMetadata.factor;
      const sourceVolumeMilliliters =
        sourceGrams / densityMatch.gramsPerMilliliter;
